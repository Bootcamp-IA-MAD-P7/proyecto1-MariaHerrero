...
def mostrar_ticket(tiempo_parado, tiempo_movimiento, tarifa_total, precio_combustible):

    print("\n🎫 TICKET DE TAXI")
    print("=" * 35)
    print(f"⛽ Combustible: €{precio_combustible}")
    print(f"🟡 Tiempo parado: {tiempo_parado:.1f}s")
    print(f"🟢 Tiempo movimiento: {tiempo_movimiento:.1f}s")
    print("-" * 35)
    print(f"💰 TOTAL: €{tarifa_total:.2f}")
    print("=" * 35)   

def animar_tarifa_final(tarifa_final):

            print("\n💶 Calculando tarifa final...")

            total = 0
            pasos = 10  # cuántos “saltos” tendrá la animación

            incremento = tarifa_final / pasos

            for i in range(pasos):
                total += incremento
                print(f"\r💰 Tarifa: €{total:.2f}", end="")
                time.sleep(0.1)

            print(f"\r💰 Tarifa final: €{tarifa_final:.2f}")
...
while True:
        ...
        if command == "inicio":
            ...
            animar_tarifa_final(tarifa_total)
